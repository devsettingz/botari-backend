...
exports.getCallSession = getCallSession;
exports.getCallSessionById = getCallSessionById;
exports.updateCallSession = updateCallSession;
...
exports.getBusinessActiveCallCount = getBusinessActiveCallCount;
exports.bridgeAIAgent = bridgeAIAgent;
exports.transferCall = transferCall;
exports.putOnHold = putOnHold;
exports.resumeFromHold = resumeFromHold;
exports.sendToVoicemail = sendToVoicemail;
exports.scheduleCallback = scheduleCallback;
exports.handleNoInput = handleNoInput;
 ...
 */
async function createCallSession(callId, callUuid, businessId, customerPhone, direction, employeeId) {
    const session = {
        callId,
        callUuid,
        businessId,
 ...
 * Update call session
 */
function updateCallSession(callUuid, updates) {
 ...
 */
function addConversationEntry(callUuid, role, content) {
    const session = activeSessions.get(callUuid);
    if (!session)
        return;
    session.conversationHistory.push({
 ...
 * End call session
 */
async function endCallSession(callUuid) {
    const session = activeSessions.get(callUuid);
    if (!session)
        return;
    // Update status
 ...
 */
function getBusinessActiveCallCount(businessId) {
    let count = 0;
    for (const session of activeSessions.values()) {
 ...
 */
async function bridgeAIAgent(callUuid, userInput, speechResult) {
    const session = activeSessions.get(callUuid);
    if (!session) {
        return [
            {
    ...
    const businessRes = await db_1.default.query('SELECT * FROM businesses WHERE id = $1', [session.businessId]);
    const business = businessRes.rows[0];
    // Get employee for this call
    const employeeRes = await db_1.default.query(`SELECT ae.* FROM business_employees be
     JOIN ai_employees ae ON be.employee_id = ae.id
     WHERE be.business_id = $1 AND be.employee_id = $2
     LIMIT 1`, [session.businessId, session.employeeId]);
    const employee = employeeRes.rows[0];
    // Determine employee type for persona
    const employeeType = employee?.name?.toLowerCase() || 'rachel';
    try {
        // Process through AI agent
        const aiResponse = await (0, agent_1.processMessage)(userInput, session.customerPhone, employeeType, {
 ...
 */
async function transferCall(callUuid, targetNumber, options = {}) {
    const session = activeSessions.get(callUuid);
    if (!session) {
        return [
            {
 ...
 */
async function resumeFromHold(callUuid, message) {
    const session = activeSessions.get(callUuid);
    if (!session) {
        return [
            {
 ...
 */
function sendToVoicemail(callUuid, reason) {
    const session = activeSessions.get(callUuid);
 ...
 * Schedule a callback
 */
async function scheduleCallback(callUuid, customerPhone, preferredTime, notes) {
    try {
        const session = activeSessions.get(callUuid);
        if (!session)
            return false;
        // Get the employee for this business
        const employeeRes = await db_1.default.query(`SELECT ae.id FROM business_employees be
       JOIN ai_employees ae ON be.employee_id = ae.id
       WHERE be.business_id = $1
       LIMIT 1`, [session.businessId]);
 ...
 */
async function handleNoInput(callUuid, attempt = 1) {
    const session = activeSessions.get(callUuid);
    if (!session) {
        return [
            {
 ...
 */
async function cleanupStaleSessions(maxAgeMinutes = 30) {
    const now = new Date();
    let cleanedCount = 0;
 ...
 */
async function getCallAnalytics(businessId, startDate, endDate) {
    try {
       ...
       WHERE business_id = $1
       AND started_at >= $2
       AND started_at <= $3`, [businessId, startDate, endDate]);
