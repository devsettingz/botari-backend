...
router.post('/context', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user.business_id || req.userId;
        const { useCase, industry, websiteUrl, businessSize, emailVolume, socialFrequency, contentFrequency, painPoints, goals, brandVoice } = req.body;
        if (!businessId) {
            return res.status(401).json({ error: 'Unauthorized' });
        }
        await db_1.default.query(`INSERT INTO business_context 
...
router.post('/analyze', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user.business_id || req.userId;
        if (!businessId) {
            return res.status(401).json({ error: 'Unauthorized' });
        ...
        // Get context
        const contextRes = await db_1.default.query('SELECT * FROM business_context WHERE business_id = $1', [businessId]);
        ...
        const hiredRes = await db_1.default.query(`SELECT be.employee_id, ae.name, ae.employee_role, ae.tier 
...
router.get('/context', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user.business_id || req.userId;
        const result = await db_1.default.query('SELECT * FROM business_context WHERE business_id = $1', [businessId]);
...
router.get('/agent-config/:employeeId', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user.business_id || req.userId;
        const { employeeId } = req.params;
        const configRes = await db_1.default.query(`SELECT ac.*, ae.name, ae.employee_role, ae.color_theme, ae.icon_emoji 
             FROM agent_configurations ac
             JOIN ai_employees ae ON ac.employee_id = ae.id
             WHERE ac.business_id = $1 AND ac.employee_id = $2`, [businessId, employeeId]);
        if (configRes.rows.length === 0) {
            // Return default config if none exists
            const empRes = await db_1.default.query('SELECT * FROM ai_employees WHERE id = $1', [employeeId]);
...
router.put('/agent-config/:employeeId', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user.business_id || req.userId;
        const { employeeId } = req.params;
        const { configuration, system_prompt } = req.body;
        await db_1.default.query(`INSERT INTO agent_configurations 
             ...
             DO UPDATE SET 
               ...
               is_configured = true,
               updated_at = CURRENT_TIMESTAMP`, [businessId, employeeId, JSON.stringify(configuration), system_prompt]);
...
async function configureAgentForBusiness(businessId, employeeId, employeeName, context) {
...
Flag risky clauses. Never provide legal advice, only document preparation and review assistance.`;
    ...
    await db_1.default.query(`INSERT INTO agent_configurations 
         ...
         DO UPDATE SET 
           ...
           updated_at = CURRENT_TIMESTAMP`, [businessId, employeeId, JSON.stringify(config), systemPrompt]);
}
function generateOptimalSchedule(context) {
    const schedules = {
...
function generateContentPillars(industry) {
    const pillars = {
...
function calculateOptimizationScore(context, agentCount) {
    let score = 0;
    if (context.email_volume === '200+')
        score += 20;
    if (context.pain_points?.length > 0)
        score += 20;
    if (agentCount > 0)
        score += 40;
    if (context.website_url && context.website_url !== 'none')
        score += 20;
    return Math.min(score, 100);
}
function calculateHoursSaved(context) {
    let hours = 0;
    if (context.email_volume === '200+')
        hours += 15;
    else if (context.email_volume === '50-200')
        hours += 8;
    if (context.social_posting_frequency === 'Never')
        hours += 10;
    if (context.pain_points?.includes('Missing leads while sleeping'))
...
function getDefaultConfig(employeeName) {
    return {
        mode: 'standard',
        auto_reply: false,
