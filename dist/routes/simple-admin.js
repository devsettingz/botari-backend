...
router.post('/login', async (req, res) => {
    ...
    // Generate token
    const token = jsonwebtoken_1.default.sign({ user_id: 'admin', email: ADMIN_EMAIL, role: 'admin', name: 'Platform Admin' }, JWT_SECRET, { expiresIn: '7d' });
...
router.get('/businesses', verifyAdmin, async (req, res) => {
    try {
        const result = await pool.query(`SELECT b.*, u.email as owner_email, 
...
router.get('/employees', verifyAdmin, async (req, res) => {
    try {
        const result = await pool.query(`SELECT e.*, COUNT(be.business_id) as business_count
       FROM employees e
...
router.get('/conversations', verifyAdmin, async (req, res) => {
    try {
        const result = await pool.query(`SELECT c.*, b.name as business_name, e.name as employee_name
...
router.get('/payments', verifyAdmin, async (req, res) => {
    ...
    try {
        const result = await pool.query(`SELECT p.*, b.name as business_name, u.email as user_email, ae.name as employee_name
...
router.get('/users', verifyAdmin, async (req, res) => {
    try {
        const result = await pool.query(`SELECT u.id, u.email, u.business_name, u.created_at, u.is_active,
...
router.get('/subscriptions', verifyAdmin, async (req, res) => {
    try {
        const result = await pool.query(`SELECT s.*, b.name as business_name
...
//# sourceMappingURL=simple-admin.js.map