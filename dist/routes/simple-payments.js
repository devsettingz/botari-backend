...
router.post('/create', verifyUser, async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO payment_approvals 
       (user_id, business_id, employee_id, amount, service_name, status, created_at)
       VALUES ($1, $2, $3, $4, $5, 'pending', NOW())
       RETURNING *`, [user_id, business_id, employee_id, amount, service_name]);
        res.json({
            success: true,
...
router.get('/my-payments', verifyUser, async (req, res) => {
    const user_id = req.user.id;
    try {
        const result = await pool.query(`SELECT * FROM payment_approvals 
       WHERE user_id = $1 
...
router.get('/my-services', verifyUser, async (req, res) => {
    const user_id = req.user.id;
    try {
        const result = await pool.query(`SELECT p.*, e.name as employee_name, e.avatar_url 
       FROM payment_approvals p
       LEFT JOIN employees e ON p.employee_id = e.id
       WHERE p.user_id = $1 AND p.status = 'approved'
       ORDER BY p.approved_at DESC`, [user_id]);
...
router.post('/admin/:id/approve', verifyAdmin, async (req, res) => {
    ...
    try {
        ...
        const result = await pool.query(`UPDATE payment_approvals 
       SET status = 'approved', 
           approved_by = $1, 
           approved_at = NOW()
       WHERE id = $2
       RETURNING *`, [admin_id, id]);
        ...
        await pool.query(`INSERT INTO business_employees (business_id, employee_id, status, hired_at)
       VALUES ($1, $2, 'active', NOW())
       ON CONFLICT (business_id, employee_id) 
       DO UPDATE SET status = 'active', hired_at = NOW()`, [payment.business_id, payment.employee_id]);
        res.json({
            success: true,
...
router.post('/admin/:id/reject', verifyAdmin, async (req, res) => {
    ...
    try {
       ...
       SET status = 'rejected', 
           ...
           rejection_reason = $2
       WHERE id = $3
       RETURNING *`, [admin_id, reason || null, id]);
        if (result.rows.length === 0) {
            return res.status(404).json({ error: 'Payment not found' });
        }
        res.json({
...
//# sourceMappingURL=simple-payments.js.map