 ...
 */
router.post('/connect', verifyToken_1.verifyToken, async (req, res) => {
    try {
        ...
        const connectionQR = await whatsapp_1.baileysManager.connect(businessId, employee_id);
        res.json({
            success: true,
            status: 'awaiting_qr',
 ...
 */
router.get('/status', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        ...
        const empId = employee_id ? parseInt(employee_id) : await getDefaultEmployeeId(businessId);
        const status = whatsapp_1.baileysManager.getStatus(businessId, empId);
 ...
 */
router.post('/disconnect', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        ...
        await whatsapp_1.baileysManager.disconnect(businessId, empId, logout);
        res.json({
            success: true,
            status: 'disconnected',
            message: logout
 ...
 */
router.post('/send', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        ...
        await whatsapp_1.baileysManager.sendMessage(businessId, to, message);
        res.json({
            success: true,
 ...
 */
router.post('/broadcast', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        ...
        const results = await whatsapp_1.baileysManager.broadcastMessage(businessId, recipients, message, delay_ms);
        res.json({
            success: true,
            sent: results.sent,
 ...
 */
router.get('/qr', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        const { employee_id } = req.query;
        if (!businessId) {
        ...
        const empId = employee_id ? parseInt(employee_id) : await getDefaultEmployeeId(businessId);
        const qrCode = whatsapp_1.baileysManager.getQRCode(businessId, empId);
        if (!qrCode) {
 ...
 */
router.get('/sessions', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        if (!businessId) {
            return res.status(401).json({ error: 'Business ID not found' });
        }
        const allSessions = whatsapp_1.baileysManager.getAllSessions();
 ...
 */
router.post('/reconnect', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        const { employee_id } = req.body;
        if (!businessId) {
        ...
        // Disconnect first
        await whatsapp_1.baileysManager.disconnect(businessId, empId, false);
        // Reconnect
        const connectionQR = await whatsapp_1.baileysManager.connect(businessId, empId);
        res.json({
            success: true,
            status: 'awaiting_qr',
            qrCode: connectionQR.qrCode,
