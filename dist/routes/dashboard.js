...
router.get('/stats', verifyToken_1.verifyToken, async (req, res) => {
    try {
        ...
        if (user.role === 'owner' || user.role === 'admin') {
            ...
            const metrics = await AnalyticsService_1.default.getOverviewMetrics();
            const platformMetrics = await AnalyticsService_1.default.getPlatformMetrics();
            res.json({
                success: true,
                data: {
                    overview: metrics,
            ...
            const metrics = await AnalyticsService_1.default.getOverviewMetrics(businessId);
            const businessMetrics = await AnalyticsService_1.default.getBusinessMetrics(businessId);
            res.json({
                success: true,
                data: {
                    overview: metrics,
...
router.get('/conversations', verifyToken_1.verifyToken, async (req, res) => {
    try {
      ...
      LIMIT 10
    `;
        const recentResult = await db_1.default.query(recentQuery, businessId ? [businessId] : []);
        res.json({
            success: true,
            data: {
                summary: {
...
router.get('/employees', verifyToken_1.verifyToken, async (req, res) => {
    try {
        ...
        startDate.setDate(startDate.getDate() - days);
        const dateRange = { startDate, endDate };
        const employeeMetrics = await AnalyticsService_1.default.getAllEmployeesMetrics(businessId, dateRange);
 ...
 */
router.get('/revenue', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const user = req.user;
        let businessId;
        if (user.role !== 'owner' && user.role !== 'admin') {
        ...
        startDate.setDate(startDate.getDate() - days);
        const dateRange = { startDate, endDate };
        const revenueMetrics = await AnalyticsService_1.default.getRevenueMetrics(businessId, dateRange);
        // Get recent orders
        let ordersQuery = `
      SELECT 
        o.id,
      ...
      LIMIT 10
    `;
        const ordersResult = await db_1.default.query(ordersQuery, businessId ? [businessId] : []);
        // Get revenue by day for chart
        const revenueByDayResult = await db_1.default.query(`
      SELECT 
 ...
 */
router.get('/:business_id/trends/messages/weekly', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const user = req.user;
 ...
 */
router.get('/:business_id/trends/conversations/weekly', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const user = req.user;
        const businessId = parseInt(req.params.business_id);
        if (user.role !== 'owner' && user.role !== 'admin' && user.business_id !== businessId) {
 ...
 */
router.get('/:business_id/summary', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const user = req.user;
        const businessId = parseInt(req.params.business_id);
        if (user.role !== 'owner' && user.role !== 'admin' && user.business_id !== businessId) {
        ...
        const [overview, businessMetrics] = await Promise.all([
            AnalyticsService_1.default.getOverviewMetrics(businessId),
            AnalyticsService_1.default.getBusinessMetrics(businessId)
        ]);
        // Get quick stats
 ...
 */
router.get('/:business_id/trends/active-users/weekly', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const user = req.user;
        const businessId = parseInt(req.params.business_id);
        if (user.role !== 'owner' && user.role !== 'admin' && user.business_id !== businessId) {
 ...
 */
router.get('/:business_id/trends/revenue/weekly', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const user = req.user;
        const businessId = parseInt(req.params.business_id);
        if (user.role !== 'owner' && user.role !== 'admin' && user.business_id !== businessId) {
