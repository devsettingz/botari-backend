...
const VonageService_1 = require("../voice/VonageService");
const CallManager_1 = require("../voice/CallManager");
const router = (0, express_1.Router)();
/**
 * POST /calls/outbound
 * Make an outbound call
 */
router.post('/outbound', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        if (!businessId) {
            return res.status(401).json({ error: 'Business ID not found' });
        }
        const { phoneNumber, employeeId, greetingText, voiceName = 'Joey', metadata } = req.body;
        if (!phoneNumber) {
        ...
        if (!targetEmployeeId) {
            const agentRes = await db_1.default.query(`SELECT be.employee_id 
        ...
        // Make the call
        const result = await (0, VonageService_1.makeOutboundCall)(phoneNumber, businessId, targetEmployeeId, {
            greetingText,
            voiceName,
...
router.get('/', verifyToken_1.verifyToken, async (req, res) => {
    try {
        ...
        const { limit = '20', offset = '0', status, startDate, endDate, direction } = req.query;
        // Build query
        ...
        // Get total count
        const countResult = await db_1.default.query(`SELECT COUNT(*) FROM (${query}) as count_query`, params);
        const total = parseInt(countResult.rows[0].count, 10);
        // Get calls with pagination
        params.push(parseInt(limit, 10));
        params.push(parseInt(offset, 10));
        const callsResult = await db_1.default.query(`${query} ORDER BY started_at DESC LIMIT $${paramIndex++} OFFSET $${paramIndex++}`, params);
        res.json({
 ...
 */
router.get('/active', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        if (!businessId) {
            return res.status(401).json({ error: 'Business ID not found' });
        ...
        const activeCalls = await (0, VonageService_1.getActiveCalls)(businessId);
        // Get active sessions (in-memory)
        const activeSessions = (0, CallManager_1.getAllActiveSessions)()
 ...
 */
router.get('/analytics', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        if (!businessId) {
            return res.status(401).json({ error: 'Business ID not found' });
        ...
        const analytics = await (0, CallManager_1.getCallAnalytics)(businessId, start, end);
       ...
       WHERE business_id = $1
       AND started_at >= $2
       AND started_at <= $3`, [businessId, start, end]);
        res.json({
            ...analytics,
            ...statsResult.rows[0],
            period: { start, end },
...
router.get('/:id', verifyToken_1.verifyToken, async (req, res) => {
    try {
        ...
        // Get call details
        const call = await (0, VonageService_1.getCallDetails)(callId, businessId);
        if (!call) {
            return res.status(404).json({ error: 'Call not found' });
        }
        // Get active session if exists
        const session = (0, CallManager_1.getCallSessionById)(callId);
        res.json({
            ...call,
 ...
 * End an active call
 */
router.post('/:id/end', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        if (!businessId) {
            return res.status(401).json({ error: 'Business ID not found' });
        ...
        // End the call
        const success = await (0, VonageService_1.endCall)(callId, call.vonage_call_uuid || undefined);
        // Also end any active session
        if (call.vonage_call_uuid) {
            await (0, CallManager_1.endCallSession)(call.vonage_call_uuid);
        }
        if (success) {
 ...
 */
router.post('/:id/record', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        if (!businessId) {
            return res.status(401).json({ error: 'Business ID not found' });
        ...
        // Start recording
        const success = await (0, VonageService_1.startRecording)(callId, call.vonage_call_uuid);
        if (success) {
 ...
 */
router.post('/:id/transfer', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        if (!businessId) {
            return res.status(401).json({ error: 'Business ID not found' });
        ...
        const ncco = await (0, CallManager_1.transferCall)(call.vonage_call_uuid, targetNumber, {
            whisperMessage: message,
        });
        // Update call metadata
        await db_1.default.query(`UPDATE calls SET metadata = jsonb_set(COALESCE(metadata, '{}'), '{transfer_initiated}', $1) WHERE id = $2`, [JSON.stringify({ targetNumber, at: new Date().toISOString() }), callId]);
        res.json({
            success: true,
 ...
 * Put a call on hold
 */
router.post('/:id/hold', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        if (!businessId) {
            return res.status(401).json({ error: 'Business ID not found' });
        ...
        // Generate hold NCCO
        const ncco = (0, CallManager_1.putOnHold)(call.vonage_call_uuid);
        res.json({
            success: true,
 ...
 */
router.post('/:id/resume', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        if (!businessId) {
            return res.status(401).json({ error: 'Business ID not found' });
        ...
        const ncco = await (0, CallManager_1.resumeFromHold)(call.vonage_call_uuid, message);
        res.json({
            success: true,
 ...
 */
router.post('/:id/summary', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const businessId = req.user?.business_id;
        if (!businessId) {
            return res.status(401).json({ error: 'Business ID not found' });
        ...
        // Generate summary
        const summary = await (0, VonageService_1.generateCallSummary)(callId);
        if (summary) {
...
router.delete('/:id', verifyToken_1.verifyToken, async (req, res) => {
    try {
        ...
        // Delete the call
        const result = await db_1.default.query('DELETE FROM calls WHERE id = $1 AND business_id = $2 RETURNING id', [callId, businessId]);
 ...
 */
router.get('/health/vonage', verifyToken_1.verifyToken, async (req, res) => {
    try {
        const health = await (0, VonageService_1.checkVonageHealth)();
        res.json(health);
    }
    catch (error) {
...
router.post('/incoming/:businessId', async (req, res) => {
    try {
        ...
        const agentRes = await db_1.default.query(`SELECT ae.* FROM business_employees be
       JOIN ai_employees ae ON be.employee_id = ae.id
       WHERE be.business_id = $1 
...
router.post('/transcribe/:businessId', async (req, res) => {
    try {
        ...
        // Save to database
        await db_1.default.query(`INSERT INTO calls 
       (business_id, customer_phone, transcript, recording_url, status, started_at, ended_at)
       VALUES ($1, $2, $3, $4, 'transcribed', NOW(), NOW())`, [businessId, From, TranscriptionText, RecordingUrl]);
        // Create follow-up reminder
        await db_1.default.query(`INSERT INTO follow_ups (business_id, customer_phone, scheduled_at, notes, channel, created_by)
       VALUES ($1, $2, NOW() + INTERVAL '1 hour', $3, 'call', 'system')`, [businessId, From, `Missed call from ${From}. Message: ${TranscriptionText}`]);
        res.sendStatus(200);
    }
    catch (error) {
...
router.get('/history', verifyToken_1.verifyToken, async (req, res) => {
    try {
        ...
        const result = await db_1.default.query(`SELECT * FROM calls WHERE business_id = $1 ORDER BY started_at DESC LIMIT 20`, [businessId]);
