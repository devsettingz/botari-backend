...
const personas_1 = require("../agent/personas");
...
class AIAgentService {
     ...
     */
    async processMessage(employeeId, message, context) {
        ...
        try {
            ...
            const conversationId = await this.getOrCreateConversation(context.customer_phone || 'unknown', context.business_id || 1, context.employee_id || 1);
            ...
            const tools = (0, actions_1.getEmployeeTools)(employeeType);
            // Build system prompt
     ...
     */
    async generateResponse(employeeId, conversationHistory, context) {
        const employeeType = this.normalizeEmployeeType(employeeId);
        const persona = personas_1.PERSONAS[employeeType];
        if (!persona) {
            throw new Error(`Unknown employee type: ${employeeId}`);
     ...
     */
    async handleWhatsAppMessage(businessId, employeeId, customerPhone, message, options) {
     ...
     */
    async handleEmail(businessId, employeeId, emailContent, options) {
        // Get business context
        const businessContext = await this.getBusinessContext(businessId);
     ...
     */
    async handleVoiceCall(businessId, employeeId, customerPhone, transcript, options) {
        // Get business context
        const businessContext = await this.getBusinessContext(businessId);
        // Resolve employee type (default to Omar or Rachel for voice)
        let employeeType = 'omar';
        if (employeeId) {
     ...
     */
    getEmployeeActions(employeeType) {
        return (0, actions_1.getEmployeeTools)(this.normalizeEmployeeType(employeeType));
    }
    /**
     * Get employee persona details
     */
    getEmployeePersona(employeeType) {
        return personas_1.PERSONAS[this.normalizeEmployeeType(employeeType)];
    }
    // ============================================================================
     ...
     */
    normalizeEmployeeType(employeeId) {
        ...
        return mappings[typeStr] || typeStr;
    }
    /**
     * Build system prompt with context
     */
    buildSystemPrompt(employeeType, context, persona) {
...
Available tools: ${persona.tools.join(', ')}`;
     ...
     */
    async getOrCreateConversation(customerPhone, businessId, employeeId) {
        try {
            const convResult = await db_1.default.query(`SELECT id FROM conversations 
         WHERE customer_phone = $1 AND business_id = $2 
         ORDER BY started_at DESC LIMIT 1`, [customerPhone, businessId]);
           ...
           VALUES ($1, $2, $3, 'open', NOW()) RETURNING id`, [businessId, customerPhone, employeeId]);
     ...
     */
    async saveMessages(conversationId, userMessage, aiReply, actions) {
        try {
            // Save user message
    ...
    async getBusinessContext(businessId) {
        try {
            const result = await db_1.default.query('SELECT id, name, description FROM businesses WHERE id = $1', [businessId]);
     ...
     */
    async resolveEmployeeType(employeeId, businessId) {
        // If it's already a valid employee type string
        const normalizedType = this.normalizeEmployeeType(String(employeeId));
        if (personas_1.PERSONAS[normalizedType]) {
            return normalizedType;
        }
        // Try to look up by ID
        try {
            const result = await db_1.default.query(`SELECT ae.name FROM ai_employees ae
         JOIN business_employees be ON ae.id = be.employee_id
         WHERE be.id = $1 OR ae.id = $1 AND be.business_id = $2`, [employeeId, businessId]);
            if (result.rows.length > 0) {
     ...
     * Get conversation history for context
     */
    async getConversationHistory(businessId, customerPhone, limit = 10) {
        try {
            const result = await db_1.default.query(`SELECT role, content FROM messages
         WHERE conversation_id IN (
           SELECT id FROM conversations 
           WHERE business_id = $1 AND customer_phone = $2
         )
         ORDER BY created_at DESC
         LIMIT $3`, [businessId, customerPhone, limit]);
     ...
     */
    async findVoiceAgent(businessId) {
        try {
            const result = await db_1.default.query(`SELECT ae.name FROM ai_employees ae
         JOIN business_employees be ON ae.id = be.employee_id
     ...
     */
    async logInteraction(businessId, employeeType, channel, input, output) {
        try {
         ...
         SET messages_processed = COALESCE(messages_processed, 0) + 1,
             last_active = NOW()
         WHERE business_id = $1 
         AND employee_id = (SELECT id FROM ai_employees WHERE name = $2)`, [businessId, employeeType]);
        }
        catch (error) {
...
// CONVENIENCE EXPORTS
// ============================================================================
async function processMessage(employeeId, message, context) {
    return exports.aiAgentService.processMessage(employeeId, message, context);
}
async function handleWhatsAppMessage(businessId, employeeId, customerPhone, message, options) {
    return exports.aiAgentService.handleWhatsAppMessage(businessId, employeeId, customerPhone, message, options);
}
async function handleEmail(businessId, employeeId, emailContent, options) {
    return exports.aiAgentService.handleEmail(businessId, employeeId, emailContent, options);
}
async function generateResponse(employeeId, conversationHistory, context) {
    return exports.aiAgentService.generateResponse(employeeId, conversationHistory, context);
}
async function executeAction(actionName, params, context) {
    return exports.aiAgentService.executeAction(actionName, params, context);
}
// ============================================================================
// DEFAULT EXPORT
// ============================================================================
exports.default = {
    aiAgentService: exports.aiAgentService,
    processMessage,
