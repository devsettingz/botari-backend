...
router.post('/', async (req, res) => {
  try {
    const { 
      From,           // Sender phone number (Twilio format)
      Body,           // Message text
      WaId,           // WhatsApp ID
    ...
    const status = baileysManager.getStatus(bizId, employeeId);
    ...
    try {
      const reply = await routeMessage(Body, phoneNumber, 'whatsapp', bizId);

      // Send reply via WhatsApp
      await baileysManager.sendMessage(bizId, phoneNumber, reply);
...
router.post('/incoming', async (req, res) => {
  try {
    ...
    await pool.query(
      ...
      [bizId, phone, status.phoneNumber || 'bot', message, 'incoming', 'delivered']
    );

    // Process through AI
    const reply = await routeMessage(message, phone, 'whatsapp', bizId);

    // Send reply
    await baileysManager.sendMessage(bizId, phone, reply);
...
router.get('/health', (req, res) => {
  res.json({
    ...
    activeSessions: baileysManager.getAllSessions().length
  });
});
 ...
 */
async function handleBaileysMessage(payload: any, res: any): Promise<any> {
  ...
  try {
    await pool.query(
      `INSERT INTO whatsapp_messages 
       (business_id, sender, recipient, content, direction, status, created_at)
       VALUES ($1, $2, $3, $4, $5, $6, NOW())`,
      [businessId, phoneNumber, 'pending', message, 'incoming', status]
    );
  } catch (error) {
