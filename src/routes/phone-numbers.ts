...
router.get('/countries', async (req, res) => {
  try {
    const result = await pool.query(
      'SELECT * FROM available_countries WHERE is_available = true ORDER BY country_name'
    );
...
router.get('/business/:businessId', verifyToken, async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT pn.*, e.display_name as employee_name
       FROM phone_numbers pn
       LEFT JOIN ai_employees e ON pn.employee_id = e.id
       WHERE pn.business_id = $1
       ORDER BY pn.is_default DESC, pn.created_at DESC`,
      [businessId]
    );
...
router.post('/purchase', verifyToken, async (req, res) => {
  ...
  try {
    ...
    // Insert new number
    const result = await pool.query(
      `INSERT INTO phone_numbers (business_id, employee_id, phone_number, country_code, country_name, is_active, is_default, monthly_cost)
       VALUES ($1, $2, $3, $4, $5, true, false, 1.00)
       RETURNING *`,
      [business_id, employee_id, phone_number, country_code, country_name]
    );
...
router.put('/:id/set-default', verifyToken, async (req, res) => {
  ...
  try {
    ...
    await pool.query(
      'UPDATE phone_numbers SET is_default = false WHERE business_id = $1',
      [business_id]
    );
    
    // Set new default
    const result = await pool.query(
      'UPDATE phone_numbers SET is_default = true WHERE id = $1 RETURNING *',
      [id]
    );
...
router.post('/webhook/inbound', async (req, res) => {
  ...
  try {
    ...
    const result = await pool.query(
      `SELECT pn.*, b.name as business_name, e.id as employee_id, e.name as employee_name
       FROM phone_numbers pn
