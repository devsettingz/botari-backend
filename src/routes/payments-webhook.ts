  ...
  try {
    ...
    const result = await pool.query(
      `INSERT INTO payments (business_id, user_id, amount, currency, provider, status)
       VALUES ($1, $2, $3, $4, $5, $6)
       RETURNING *`,
      [businessId, userId, amount, currency, provider, status]
    );

    if (status === 'success' && businessId) {
      await pool.query(
        `INSERT INTO subscriptions (business_id, user_id, plan, status, started_at, expires_at)
         VALUES ($1, $2, 'pro', 'active', NOW(), NOW() + interval '30 days')
         ON CONFLICT (business_id, user_id)
         DO UPDATE SET status = 'active', updated_at = NOW(), expires_at = NOW() + interval '30 days'`,
        [businessId, userId]
      );
    }
