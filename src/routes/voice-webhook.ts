...
router.get('/answer', async (req, res) => {
  await handleAnswerWebhook(req, res);
});
...
router.post('/event', async (req, res) => {
  ...
  if (uuid) {
    ...
    const { getCallSession } = await import('../voice/CallManager');
    const session = getCallSession(uuid);
    if (session) {
      (req.params as any).callId = session.callId.toString();
      await handleEventWebhook(req, res);
      return;
    }
  }
...
router.post('/input/:callId', async (req, res) => {
  await handleInputWebhook(req, res);
});
...
router.post('/recording/:callId', async (req, res) => {
  await handleRecordingWebhook(req, res);
});
...
router.post('/voicemail/:callId', async (req, res) => {
  await handleVoicemailWebhook(req, res);
});
...
router.post('/machine-detection', async (req, res) => {
  await handleMachineDetectionWebhook(req, res);
});
...
router.get('/health', async (req, res) => {
  try {
    const { checkVonageHealth } = await import('../voice/VonageService');
