...
export interface DailyReport {
  ...
  hourlyBreakdown: Array<{ hour: number; conversations: number; messages: number }>;
}
...
export class ReportGenerator {
  ...
  async generateDailyReport(businessId?: number, date?: Date): Promise<DailyReport> {
    ...
    const dateRange: DateRange = { startDate: startOfDay, endDate: endOfDay };
    ...
    const employeeActivityResult = await pool.query(`
      SELECT 
        ae.display_name as employee_name,
  ...
  async generateWeeklyReport(businessId?: number, date?: Date): Promise<WeeklyReport> {
  ...
  async generateMonthlyPlatformReport(year?: number, month?: number): Promise<MonthlyPlatformReport> {
    const now = new Date();
    ...
    // Get business growth
    const newBusinessesResult = await pool.query(`
      SELECT COUNT(*) as count FROM businesses
      WHERE created_at >= $1 AND created_at <= $2
    `, [startDate, endDate]);

    const churnedBusinessesResult = await pool.query(`
      SELECT COUNT(*) as count FROM businesses
    ...
    // Get top businesses
    const topBusinessesResult = await pool.query(`
      SELECT 
  ...
  async generateBusinessHealthScore(businessId: number): Promise<BusinessHealthScore> {
    ...
    else if (overallScore < 75) status = 'needs_attention';
  ...
  private generateInsights(
    metrics: BusinessMetrics | PlatformMetrics,
