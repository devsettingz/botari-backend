...
export interface BusinessMetrics {
  ...
  topProducts: Array<{ name: string; inquiries: number }>;
  trends: {
...
export interface RevenueMetrics {
  ...
  revenueByPlan: Array<{ plan: string; revenue: number; businesses: number }>;
...
export class AnalyticsService {
  ...
  async getOverviewMetrics(businessId?: number): Promise<OverviewMetrics> {
    ...
    const [conversationsResult, messagesResult, businessesResult, revenueResult] = await Promise.all([
      pool.query(`SELECT COUNT(*) as count FROM conversations ${whereClause}`, params),
      pool.query(`SELECT COUNT(*) as count FROM messages ${whereClause}`, params),
      pool.query(`SELECT COUNT(DISTINCT id) as count FROM businesses WHERE is_active = true`),
      pool.query(`SELECT COALESCE(SUM(amount), 0) as total FROM payments WHERE status = 'completed' ${businessId ? 'AND business_id = $1' : ''}`, businessId ? [businessId] : [])
    ]);
  ...
  async getBusinessMetrics(businessId: number, dateRange?: DateRange): Promise<BusinessMetrics> {
    const cacheKey = this.getCacheKey('business', { businessId, dateRange });
    const cached = this.getCached<BusinessMetrics>(cacheKey);
    if (cached) return cached;
  ...
  private async getBusinessTrends(businessId: number, dateRange?: DateRange): Promise<{ conversations: TimeSeriesPoint[]; messages: TimeSeriesPoint[] }> {
  ...
  async getEmployeeMetrics(employeeId: number, businessId?: number, dateRange?: DateRange): Promise<EmployeeMetrics> {
    const cacheKey = this.getCacheKey('employee', { employeeId, businessId, dateRange });
    const cached = this.getCached<EmployeeMetrics>(cacheKey);
    if (cached) return cached;
    ...
    const employeeResult = await pool.query(
      'SELECT id, display_name, employee_role FROM ai_employees WHERE id = $1',
      [employeeId]
    );
    ...
    const conversationResult = await pool.query(conversationQuery, conversationParams);
    ...
    `;
    const actionParams: any[] = [employee.display_name.toLowerCase(), ...dateFilterActions.params];
    ...
    const handlingTimeResult = await pool.query(`
  ...
  private async getEmployeeTrend(employeeId: number, businessId?: number, dateRange?: DateRange): Promise<TimeSeriesPoint[]> {
    const days = this.getDaysFromRange(dateRange, 30);
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - days);
    ...
    let query = `
      ...
      ORDER BY date
    `;
    const params: any[] = [employeeId, startDate];
  ...
  async getAllEmployeesMetrics(businessId?: number, dateRange?: DateRange): Promise<EmployeeMetrics[]> {
    ...
    const metrics = await Promise.all(
      result.rows.map(row => this.getEmployeeMetrics(row.id, businessId, dateRange))
    );
  ...
  async getPlatformMetrics(dateRange?: DateRange): Promise<PlatformMetrics> {
    ...
    const cached = this.getCached<PlatformMetrics>(cacheKey);
    if (cached) return cached;
  ...
  private async getBusinessGrowthTrend(dateRange?: DateRange): Promise<TimeSeriesPoint[]> {
  ...
  private async getRevenueGrowthTrend(dateRange?: DateRange): Promise<TimeSeriesPoint[]> {
    const days = this.getDaysFromRange(dateRange, 90);
    const startDate = new Date();
    startDate.setDate(startDate.getDate() - days);
  ...
  async getRevenueMetrics(businessId?: number, dateRange?: DateRange): Promise<RevenueMetrics> {
    ...
    // Get ARPU
    const arpuResult = await pool.query(`
      SELECT 
  ...
  async getConversationMetrics(businessId?: number, dateRange?: DateRange): Promise<ConversationMetrics> {
    const cacheKey = this.getCacheKey('conversations', { businessId, dateRange });
    const cached = this.getCached<ConversationMetrics>(cacheKey);
    if (cached) return cached;
    ...
    const messagesResult = await pool.query(messageQuery, messageParams);
  ...
  async exportData(
    type: 'conversations' | 'messages' | 'orders' | 'employees' | 'revenue',
    format: 'csv' | 'json',
    businessId?: number,
    dateRange?: DateRange
  ): Promise<string> {
    ...
    const result = await pool.query(query, [...params, ...dateFilter.params]);
  ...
  private buildDateFilter(dateRange?: DateRange, dateColumn: string = 'created_at'): { sql: string; params: any[] } {
  ...
  private getDaysFromRange(dateRange?: DateRange, defaultDays: number = 30): number {
    if (!dateRange?.startDate) return defaultDays;
  ...
  private fillDateGaps(data: TimeSeriesPoint[], days: number): TimeSeriesPoint[] {
